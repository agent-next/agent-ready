...
import type { Level, Pillar } from 'agent-ready';
...
export class EvaluatorAgent {
  evaluate(pillarResults: PillarAgentResult[]): Evaluation {
    ...
    const { strengths, weaknesses } = this.identifyStrengthsWeaknesses(pillarResults);
  ...
  private generateCrossPillarInsights(pillarResults: PillarAgentResult[]): CrossPillarInsight[] {
    ...
    if (test && build && test.score < 50 && build.score < 50) {
      insights.push({
        type: 'risk',
        pillars: ['test', 'build'],
        insight: 'Low test coverage with no CI/CD detected - high risk for code changes',
        recommendation: 'Prioritize CI pipeline setup and add basic tests',
      });
    }
    ...
    if (docs && env && docs.score > 70 && env.score < 50) {
      insights.push({
        type: 'opportunity',
        pillars: ['docs', 'env'],
        insight: 'Good docs but missing dev environment setup - onboarding friction',
        recommendation: 'Add devcontainer or docker-compose configuration',
      });
    }
    ...
    if (security && style && security.score > 80 && style.score > 80) {
      insights.push({
        ...
        insight: 'Strong security and code style configurations - high code quality',
  ...
  private identifyStrengthsWeaknesses(pillarResults: PillarAgentResult[]): {
    strengths: string[];
    weaknesses: string[];
  } {
    ...
    for (const pillar of sorted.slice(0, 3)) {
      if (pillar.score >= 70) {
        const name = i18n.pillars[pillar.pillar];
  ...
  private calculateTechDebtScore(pillarResults: PillarAgentResult[]): number {
    ...
    if (pillarResults.length === 0) return 0;
