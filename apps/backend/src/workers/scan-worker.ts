...
const worker = new Worker<ScanJob, ScanResult>(
  ...
  async (job: Job<ScanJob>) => {
    ...
    const { scanId, repoUrl, branch, profile } = job.data;
    const gitService = new GitService();

    console.log(`[Worker] Starting scan ${scanId} for ${repoUrl}`);

    try {
      // Update database: cloning
      await db.updateScanStatus(scanId, 'cloning', { started_at: true });
      ...
      try {
        ...
        await db.completeScan(scanId, result, durationMs);
      ...
      // Persist failure to database (wrapped to prevent stuck scans)
      try {
        await db.failScan(scanId, errorMessage, durationMs);
      } catch (dbError) {
        console.error(`[Worker] Failed to persist failure for ${scanId}:`, dbError);
      }
