...
class DatabaseService {
  ...
  async createScan(scan: {
    ...
    const result = await this.pool.query<ScanRecord>(
      `INSERT INTO scans (id, repo_url, branch, profile, status, created_at)
       VALUES ($1, $2, $3, $4, 'queued', NOW())
       RETURNING *`,
      [scan.id, scan.repo_url, scan.branch || null, scan.profile]
    );
  ...
  async getScanByCommit(commitSha: string): Promise<ScanRecord | null> {
    const result = await this.pool.query<ScanRecord>(
      `SELECT * FROM scans
       WHERE result->>'meta' IS NOT NULL
         AND result->'meta'->>'commit' = $1
