import type { CheckConfig, CheckResult, ScanContext, Pillar, Level } from 'agent-ready';
import { executeCheck, LEVELS } from 'agent-ready';
...
export abstract class BaseAgent {
  ...
  protected calculateLevelAchieved(results: CheckResult[]): Level | null {
    for (const level of LEVELS) {
      ...
      // 80% threshold + all required must pass (Factory.ai compatible)
      const passRate = passed / levelChecks.length;
      if (passRate < 0.8 || requiredPassed < requiredTotal) {
